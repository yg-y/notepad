...
public class TimeUtil {
    ...
    public static String getYearAndMonth(Long timeStamp) {
        return timeStamp2Date(String.valueOf(timeStamp), "yyyy-MM");
    }
     ...
     * @return
     */
    public static long stampToTime(String time, String format) {
        if (format == null) {
            return 0;
        }
        SimpleDateFormat sdf = new SimpleDateFormat(format);
    ...
    public static String getMonthLastDay(String date) {
        ...
        try {
            cal.setTime(dateFormatMonth.parse(date));
        } catch (ParseException e) {
            System.err.println("getMonthLastDay 格式化日期失败");
        }
     ...
     * @return
     */
    public static long appointedDay(Integer beforeDay) {
        LocalDateTime data = LocalDateTime.of(java.time.LocalDate.now(), LocalTime.MAX).minusDays(beforeDay);
        return data.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
    }
}
